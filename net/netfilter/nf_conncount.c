...
/* XXX: now nft_connlimit use this gc funciton, does it expect gc to early
 * exit? Or shall we provide a flag in this function for non early exit case? */
	    ...
	    const struct nf_conntrack_zone *zone)
{
	...
	INIT_LIST_HEAD(&rbconn->list.head);
	spin_lock_init(&rbconn->list.list_lock);
	rbconn->list.dead = false;
